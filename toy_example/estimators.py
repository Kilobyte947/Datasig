...
def _norm(v, norm, dim=-1):
    """Ordinary straight-line distance between two points. Used by pairwise_lipschitz and
    local_perturbation_lipschitz's plain-distance fallback."""
    if norm == "l2":
...
def pairwise_lipschitz(x, y, norm="l2", max_pairs=None, seed=None, embed_fn=None, precision=None):
    """L_hat = max_{i != j} |y_i - y_j| / d(x_i, x_j), over all pairs (or a random subsample if there
    are more than max_pairs). x is (N, d), y is (N,) scalar outputs. A global, not local, estimate.
    
    d is plain distance in raw x by default. If embed_fn and precision are both given, x is first
    mapped through embed_fn and d becomes the Mahalanobis distance in that embedded space; norm is
...
def _sample_in_ball(center, radius, n_samples, generator, max_attempts=10000):
    """n_samples points sampled uniformly within a ball of radius around the origin. 
...
def local_perturbation_lipschitz(f, x0, radius, n_samples, norm="l2", seed=None, embed_fn=None, precision=None):
    """Samples n_samples points within a ball of radius around x0, evaluates f at x0 and each 
    sample, and returns the largest |f(x')-f(x0)| / d(x0,x') along with the maximizing x'. f must
    accept a batch (N, d) and return a batch of scalars (N,).
    
    d follows the same embed_fn/precision convention as pairwise_lipschitz."""
...
def gradient_norm_estimate(f, x0, norm="l2"):
    """Autograd-based local Lipschitz estimate: the exact gradient norm at x0, no perturbation needed."""
