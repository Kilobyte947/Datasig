...
(`mnist_example`): a deterministic, class-balanced subset of the MNIST test
...
def load_eval_pool(n_per_class: int = 100, seed: int = 0,
    ...
    images: (10 * n_per_class, 28, 28) `torch.get_default_dtype()` in [0, 1]
    ...
    Cast to `torch.get_default_dtype()` explicitly (not left to torchvision,
    which always returns raw uint8/float32 regardless of torch's global
    default) - this package's models (the shared mnist_example checkpoint)
    are float64, so an uncast float32 pool would raise a dtype mismatch the
    moment any model's forward pass runs on it.
    """
