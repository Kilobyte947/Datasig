"""Classifier models, training loop, and MNIST loading for signature_distance's adversarial-
evaluation infrastructure.

SmallCNN and StrongCNN are imported directly from mnist_example, and every adversarial-eval
driver in this package loads mnist_example's shared, cached checkpoint rather than training its
own — so results here are computed on literally the same trained weights as mnist_example, not
just the same architecture retrained independently.
"""
...
def load_mnist_train_test(batch_size: int = 256):
    """Fresh MNIST train/test DataLoader pair."""
...
def train_classifier(model, train_loader, test_loader, epochs: int, lr: float = 1e-3,
    ...
    """Cross-entropy + Adam training loop. Returns (model, train_acc, test_acc)."""
...
def train_or_load_small_cnn(seed: int = 0, checkpoint_dir=MNIST_EXAMPLE_CHECKPOINT_DIR, verbose: bool = True):
    """Loads mnist_example's canonical trained SmallCNN checkpoint — never trains. 
    Raises FileNotFoundError if the checkpoint doesn't exist yet; train it via mnist_example first. 
    Returns (model, train_acc, test_acc), model in eval mode."""
...
def train_or_load_strong_cnn(seed: int = 0, checkpoint_dir=MNIST_EXAMPLE_CHECKPOINT_DIR, verbose: bool = True):
    """Loads mnist_example's canonical trained StrongCNN checkpoint — never trains. A shared 
    checkpoint matters here since StrongCNN's batch norm and dropout make independent training runs
    not bit-reproducible even at a fixed seed. Raises FileNotFoundError if the checkpoint doesn't exist yet. 
    Returns (model, train_acc, test_acc), model in eval mode."""
