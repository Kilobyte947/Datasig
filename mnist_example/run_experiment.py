...
from mnist_example.models import (
    LogisticRegressionModel, SmallMLP, SmallCNN, STRONG_CNN_CONFIG,
    FlattenedInputWrapper, train_classifier, margin_fn, DEVICE,
    train_or_load_small_cnn, train_or_load_strong_cnn,
)
...
from mnist_example.distance import (
    ...
    knn_label_purity, class_separation_ratio,
)
from mnist_example.embeddings import elementwise_embedding, local_patch_cross_terms
from mnist_example.smoothing import smoothed_cross_terms_embedding, gaussian_blur_embedding, RADIUS_MULTIPLIER
from mnist_example.plots import (
...
def run_smoothing_sweep(
    ...
    for sigma in sigmas:
        ...
        purity = knn_label_purity(embed_fn(x_val), y_val, k=5)
...
def run_truncated_mahalanobis_sweep(
    ...
    for name, embed_fn in feature_spaces.items():
        ...
        for k, cond in zip(k_values, cond_numbers):
            ...
            purity = knn_label_purity(whitened_val, y_val, k=5)
...
def run_stronger_cnn_raw_mnist_experiment(
    ...
    cnn_model_raw, cnn_train_acc, cnn_test_acc = train_or_load_strong_cnn(seed=seed, verbose=verbose)
