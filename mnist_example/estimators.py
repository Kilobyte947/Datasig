"""The three Lipschitz sub-methods, generalized from toy_example/estimators.py
...
def pairwise_lipschitz(model, x_batch, y_batch, output_fn, distance_fn=euclidean_distance_fn,
    ...
    toy_example's interface) as a safety valve if a caller passes a
...
def local_perturbation_lipschitz(model, x_batch, y_batch, output_fn, distance_fn=euclidean_distance_fn,
    ...
    with toy_example's `_grid` convention.
    """
    generator = _generator(seed)
    N, d = x_batch.shape
