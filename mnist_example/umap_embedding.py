...
wanted here is left as an open design choice, not decided in
...
`notebook_distance_measures.ipynb`'s UMAP section's intro markdown for the same note in context.
...
def fit_umap_embedding(x_flat, n_components=5, seed=SEED, n_neighbors=15, min_dist=0.1, verbose=False):
    ...
    **Fit/evaluate separation is the caller's responsibility, and matters**:
    ...
    supposedly-held-out distances are measured on them. Confirmed directly that `notebook_distance_measures.ipynb`'s UMAP section avoids this: `dev`, the
...
def make_umap_embed_fn(reducer):
    ...
    `tests/test_umap_embedding.py`), the same
...
# knn_label_purity now lives in distance.py (imported above) - shared with
# run_experiment.py's smoothing sweep rather than duplicated.
