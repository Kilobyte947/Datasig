...
# toy_example.run_experiment.run_gap_N_sweep_seed_averaged for the same precedent).
#
...
def run_transfer_attack(models_by_seed, test, pool, mahalanobis_distance_fn, source_seed=0,
    ...
    directly in `tests/test_adversarial_strong_cnn_seed_sweep.py`) -- attacking a model and
...
# `notebook_adversarial.ipynb's Section 4` (Checkpoint 7) stays a thin visualization driver with no
# logic of its own, per this project's convention (see distance_measures.md's "notebooks are
# thin drivers" note).
# ---------------------------------------------------------------------------
