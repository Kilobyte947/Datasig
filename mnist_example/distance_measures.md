# Distance Measures: Euclidean vs. Mahalanobis on MNIST Classifiers

Scales the three Lipschitz estimators built for a toy regression problem
(`toy_example`, sibling folder) up to real classifiers trained on MNIST,
...
| `layer_decomposition.py` | The layer-decomposition sub-experiment (see its own section below): `layer_decomposition_experiment`, `run_cnn_width_sweep`, `extractor_output_fn`/`head_output_fn`/`full_logits_output_fn`, `fit_feature_normalizer`, `linear_layer_lipschitz`-based exact head bound. Built on by `adversarial.md`'s bound-vs-attack comparison. |
| `tests/test_layer_decomposition.py` | The submultiplicative bound holds and the estimated looseness ratio is self-consistent; the estimated ratio never exceeds the exact one; the bound holds across the width sweep; unknown-method rejection. |
| `notebook_distance_measures.ipynb` | Thin driver notebook covering this whole file — the main experiment (imports from this package, runs `run_mnist_experiment()` and the opt-in `run_embedding_degree_sweep()`), then UMAP, then smoothing/`radius_multiplier`/truncated-Mahalanobis, as sequential sections with their own introductory markdown. No reusable logic of its own; the layer-decomposition sub-experiment's own notebook section lives in the sibling `notebook_adversarial.ipynb` instead (see that doc). |
...
- **ReLU for the MLP, not tanh.** `toy_example` used tanh throughout for continuity with its smooth closed-form ground truth; there's no such ground truth here, and ReLU is the standard choice for MNIST classifiers (faster to train, no vanishing-gradient concern at this depth). `SmallMLP` still supports `activation="tanh"` for anyone who wants to compare.
...
- **Ridge regularization, not PCA truncation, for the singular pixel covariance.** Ridge is the more direct generalization of `toy_example/embeddings.py`'s existing `precision_from_covariance` (which already adds `eps*I`) and needs no extra machinery — choosing a truncation rank, deciding how to handle the discarded subspace in the distance formula. It hasn't been compared head-to-head against PCA truncation (see [Limitations](#limitations-and-open-questions)).
...
.venv/bin/python -c "from mnist_example.run_experiment import run_embedding_degree_sweep; run_embedding_degree_sweep(degrees=(1, 2, 3))"

# or execute the notebook end-to-end (~1 minute on CPU for run_mnist_experiment() alone;
# meaningfully longer with the embedding-degree sweep, UMAP, and smoothing sections included)
.venv/bin/jupyter nbconvert --to notebook --execute --inplace mnist_example/notebook_distance_measures.ipynb
```

As of this reorganization (the `adversarial/` subfolder flattened into this same package),
`mnist_example/tests/` has **186 tests** in total, of which **112** are the distance-measures
side documented in this file (the rest are `adversarial.md`'s). Treat the exact count as something
that drifts as tests are added rather than a fixed number to remember -- run the command above for
the current figure.
...
`1/eigenvalue`) — measured directly (logistic regression, see the Appendix below)
...
from an unexpected source. See the Appendix below for the full pair-by-pair table,
...
got updated after a later, unrelated commit changed an upstream default (see the Appendix below
...
**Known numerical fragility, newly discovered during a re-run of this sweep**: at `degree=2`,
`epsilon=1` (one of the seven candidate epsilons `epsilon_stability_check` scans before settling
on the `0.01` reported above — not the value actually used for the table), `torch.linalg.pinv`'s
internal SVD occasionally fails to converge on one specific per-point pullback matrix
(`_LinAlgError: ... ill-conditioned or has too many repeated singular values`), raising instead of
returning a value. Confirmed this is not a regression from any code moved/renamed/deduplicated in
this reorganization pass (the numerical logic in `gradient_norm_estimate`/`svd_ridge_precision` was
not touched) — an older saved run of this exact notebook cell, same seed, completed this exact
computation successfully, so the underlying SVD implementation's convergence behavior on this
borderline-conditioned matrix appears to be environment/library-version-sensitive rather than
deterministically broken. The epsilon actually selected and used for the table above (`0.01`) is
computed earlier in the same sweep and is unaffected. Left as an open item, in the same spirit as
the `radius_multiplier`/truncated-Mahalanobis numerical findings elsewhere in this file — fixing
`gradient_norm_estimate`'s SVD-based `pinv` to be robust to this (e.g. a fallback or a
better-conditioned formulation) is real numerical-methods work, not attempted here.
...
the Appendix below for the full setup, comparison table, and a
reproducible snippet.

## Sub-experiment: layer decomposition (verdict: the per-layer bound is loose, and more so at larger widths)

`layer_decomposition.py`/`notebook_adversarial.ipynb`'s Section 1 asks a different question from the
metric comparisons above: not "which distance metric gives a better Lipschitz estimate," but "how
does a network's end-to-end Lipschitz constant relate to its individual layers'?" It splits
`SmallCNN` into `f = head o extractor` (the conv/pool stack, then the final linear layer) and
checks the classic submultiplicative bound (Szegedy et al., 2014, *"Intriguing Properties of
Neural Networks"*): `L_extractor * L_head >= L_full`.

`L_head` has a closed form (`estimators.py::linear_layer_lipschitz`, the exact spectral norm of
the final linear layer) — the one piece of this sub-experiment with a genuine ground truth, unlike
everything else in this file. `L_extractor` and `L_full` are both estimated the same way as
everywhere else in this project (pairwise/local-perturbation/gradient-norm), applied to the raw
10-d logit vector rather than the scalar margin (`layer_decomposition_experiment`'s own docstring
explains why the margin function doesn't fit this particular bound). `run_cnn_width_sweep` repeats
the comparison across five `SmallCNN` widths.

**The bound holds but is loose, and gets looser as the network gets wider** — the same Euclidean
numbers `adversarial.md`'s width-sweep table reports (`compute_bounds_with_distance_fn` is checked
directly, in `tests/test_adversarial_mahalanobis.py`, to reduce to exactly this module's own
numbers when handed plain Euclidean distance, so the two are one finding, not two):

| width | `L_head_exact` | `L_extractor_estimated` | `looseness_ratio` (`product / L_full_estimated`) |
|---|---|---|---|
| 4  | 1.590 | 4.478  | 2.33x |
| 8  | 1.336 | 6.600  | 2.70x |
| 16 | 1.090 | 9.100  | 3.10x |
| 32 | 0.992 | 13.013 | 3.43x |
| 64 | 1.005 | 26.681 | 6.13x |

The product bound overestimates the network's true end-to-end sensitivity by 2.3x at the smallest
width tested and by over 6x at the largest — capacity buys the network more ways to have its
per-layer worst directions *not* compose, not fewer. See `adversarial.md` for what this means in
practice (whether real FGSM/PGD attacks actually approach the tight bound, the loose bound, or
neither) and for the same comparison repeated under Mahalanobis distance.

## Sub-experiment: UMAP as an alternative distance metric (verdict: artifact, not a real signal)

`umap_embedding.py`/`notebook_distance_measures.ipynb`'s UMAP section (a dedicated module + thin driver notebook, same
...
the UMAP section, not by assumption; see `fit_umap_embedding`'s docstring for the checked
invariant.

See `notebook_distance_measures.ipynb`'s UMAP section ("Verdict: artifact") for the full investigation, numbers, and
image evidence. Separately: this sub-experiment uses **unsupervised** UMAP only — whether
supervised UMAP (using class labels during fitting) was the intended variant is left as an open design choice, not decided here.
...
`smoothing.py`/`notebook_distance_measures.ipynb`'s smoothing section follows up directly on the `local_patch_cross_terms`
...
`local_patch_cross_terms` itself). That section reruns the same
...
(below 1, the same low-variance-direction-amplification pattern the Appendix
...
isn't carrying new signal beyond that. See the smoothing section's "Overall verdict" section
...
(See the smoothing section's class-separation section for the underlying within/between mean
...
This is consistent with the Appendix's mechanism analysis: Mahalanobis distance amplifies
...
a new anomaly.** The Appendix's eigenbasis investigation confirmed
...
## Appendix: three follow-up analyses referenced above

Three one-off analyses referenced earlier in this document, each a completed, self-contained
check rather than a permanent part of `run_experiment.py`/`main()`.

### Why the Euclidean-vs-Mahalanobis ratio flip happens

Direct measurement of the mechanism behind the near/all ratio reversal discussed above. On
logistic regression's already-computed near-neighbor and all-pairs populations, using the
already-selected `epsilon=0.01` precision matrix:

**Amplification factor** (`mahalanobis_dist / euclidean_dist`, per pair):
...
Near-neighbor pairs are amplified 46.2% more than all-pairs (3.299 vs. 2.257 mean amplification).

**Where the difference lives in the covariance eigenbasis** — each pair's pixel-difference vector
projected onto the covariance eigenbasis (rank 1 = highest variance), cumulative fraction of
squared difference-norm explained by the top-k eigenvectors:

| Rank k | All-pairs cumulative fraction | Near-neighbor cumulative fraction |
|---|---|---|
| 1 | 0.089 | 0.026 |
| 5 | 0.319 | 0.114 |
| 10 | 0.473 | 0.214 |
...
Near-neighbor pairs explain only 61.7% of their squared difference-norm in the top 50
highest-variance directions, versus 82.1% for all-pairs — roughly twice as much of a
near-neighbor pair's difference sits outside the top 50 directions. (The literal bottom 50 of 784
eigenvectors are a dead-zero region for MNIST's covariance — constant-zero border pixels — so both
populations' bottom-50 fractions are floating-point noise, not a meaningful comparison on their
own; the cumulative curve above is the robust evidence.)

**Conclusion**: near-neighbor pairs' pixel differences are weighted more toward low/mid-variance
directions than a random pair's, which is sufficient on its own to explain the reversal —
Mahalanobis distance inflates near-neighbor pairs' distances (the ratio's denominator)
disproportionately more than a random pair's, shrinking their ratio disproportionately more.

Reproducing this (no permanent driver added — `plots.py::plot_variance_explained_curve` is the one
reusable piece; everything else is a standalone script):
...
from mnist_example.distance import euclidean_distance_fn, svd_ridge_precision, mahalanobis_distance
from mnist_example.plots import plot_variance_explained_curve

EPSILON = 0.01  # already-selected epsilon for logistic regression, see above

train = load_mnist(train=True)
test = load_mnist(train=False)

d = np.load("mnist_example/results/mnist_experiment_arrays.npz")
...
### Label-error cross-reference (validity check)

Cross-references the near-neighbor pairs flagged as high-ratio against
[cleanlab/label-errors](https://github.com/cleanlab/label-errors)'s published, human-verified
MNIST test-set label corrections (15 confirmed errors out of 10,000 test images,
`data/known_label_errors_mnist_test.json`) — no retraining, no dataset modification, purely a
cross-reference of already-computed near-neighbor pair indices.
...
**Of the 22 pairs checked, exactly one involves a known label error**: the original CNN's rank-3
pair, test index 9679 — its original label "6" was rejected by a majority of mTurk workers. This
is a positive result, not just a clean bill of health: it's direct evidence the near-neighbor
diagnostic can surface genuine *labeling* problems, not just model confusions. Every other pair,
including logistic regression's top-6 (almost entirely same-digit "1" vs. "1" pairs rather than
cross-digit confusions), checks out clean, supporting genuine model behavior as the general
pattern. One recurring point, not six independent findings: test index 5642 is a member of 5 of
logistic regression's 6 pairs — one boundary-sitting image compared against five different
neighbors, not five separate discoveries.

An earlier version of this cross-reference checked a different set of pairs ("8/2"/"8/0" for the
CNN, "0/7" for logistic regression) that don't reproduce under the current codebase: those were
generated when `run_mnist_experiment`'s `n_lipschitz_points` parameter defaulted to 300; a later,
unrelated commit changed the default to 1000, which changes the near-neighbor stratified subset's
composition (a different `exclude_idx` set) even at an identical seed. Model training itself is
unaffected by this parameter and remains bit-for-bit reproducible — confirmed by rerunning
`run_mnist_experiment()` and matching the accuracy table above to 4 decimal places. Only the
near-neighbor subset composition shifted; this is stale documentation from a superseded default,
not file corruption or non-determinism.

### `local_patch_cross_terms` under plain Euclidean distance (follow-up)

Closes an open question left by the `local_patch_cross_terms` negative result above: does the
locality signal survive under plain Euclidean distance (no covariance/precision/epsilon at all),
or was the embedding itself uninformative? Setup: a freshly trained `LogisticRegressionModel` (15
epochs, matching the baseline exactly), `euclidean_distance_fn` applied directly to
`local_patch_cross_terms`-embedded features, 1000-point class-stratified subset,
`max_pairs=20000` (a random subsample of the ~499,500 total pairs, needed to stay under ~1.5GB
peak memory at this embedding's 3920 dimensions).

| Setup | All-pairs mean | Near-neighbor mean | Near/all | Direction |
|---|---|---|---|---|
| Raw-pixel, Euclidean, logistic regression | 0.310 | 0.349 | 1.13 | above (+13%) |
| `local_patch_cross_terms`, Mahalanobis (epsilon=1 fallback, n=300) | 0.288 | 0.246 | 0.854 | below (-14.6%) |
| `local_patch_cross_terms`, plain Euclidean (n=1000, max_pairs=20000) | 0.165 | 0.171 | 1.039 | above (+3.9%) |

**Conclusion**: the locality signal does show up under plain Euclidean distance on its own (same
direction as raw-pixel Euclidean), so the earlier epsilon-selection instability was a
Mahalanobis/covariance-reweighting problem, not evidence the embedding itself is uninformative.
The signal is markedly weaker than raw pixels' own (+3.9% vs. +13%), so this embedding doesn't add
value over raw pixels here. Even the heavily-damped `epsilon=1` Mahalanobis fallback — already
noted as behaving considerably more like Euclidean distance than a genuine data-fit metric —
still reverses the near/all sign relative to true Euclidean distance on the same features (-14.6%
vs. +3.9%), extending the "self-referential covariance reweighting shrinks/reverses the signal"
finding beyond just full-strength or numerically unstable Mahalanobis.

One methodological asterisk: the Mahalanobis row caps for memory by *point count* (n=300,
exhaustive pairs), while the Euclidean row caps by *pair count* (full 1000-point pool, a 20,000-pair
subsample) — the all-pairs means are directionally but not bit-for-bit comparable between the two;
the near-neighbor side is on firmer footing (5,000 pairs here vs. ~1,500 there, both drawn from
their full point pools).

Reproducing this (a standalone script, not wired into `run_experiment.py`/`main()`):
...
from mnist_example.run_experiment import run_ratio_distribution_analysis, SEED

MAX_PAIRS = 20000  # see the memory reasoning above
...
- ~~Why the ratio-distribution flip (see above) reverses direction between Euclidean and
  ...
  Results section above and the Appendix for the full measurement.
